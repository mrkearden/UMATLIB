...
!------------------------------------------------------------------------------
  ...
  LOGICAL :: Parallel, Scanning, f1exist
  ...
  INTEGER :: NPROPS, NSTATEV, N_Gauss, noel, inpt
...
if (int(MaterialConstants(6,1)).eq.1) then
inquire(unit=40, opened=f1exist) 
if ( f1exist ) then
...
!
 SELECT CASE(CoordinateSystemDimension())
    CASE(2)
     term1=astress(1)-astress(2) 
     term2 = (0.5*term1)**2
     term3 = (astress(4))**2
     sprinmax = term1+sqrt(term2+term3)
     sprinmin = term1-sqrt(term2+term3)
     mises = sqrt(astress(1)**2-astress(1)*astress(2)+astress(2)**2+3.*astress(4)**2)
...
!------------------------------------------------------------------------------
    ...
    INTEGER :: n, nd, ntot, dofs,inpt, noel
    TYPE(Nodes_t) :: Nodes
...
!        EigenC = MATMUL( TRANSPOSE(QWork), MATMUL(C,QWork) )       
      ...
      !
      !write(*,*) ElementIndex,int(Inprops(7)),t,int(Inprops(8)),astress(1)
